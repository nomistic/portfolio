...
class WorkRepository extends ServiceEntityRepository
{
    ...
    public function totalByClient() {
        ...
        $works = $em->createQuery(
            'select c.name, c.id as cid, sum(w.net_pay) AS amount, count(w.id) as jobs, sum(w.net_pay)/count(w.id) as average_pay, c.status
            FROM App\Entity\Work w
...
// using raw SQL for monthname and year functions
    ...
    public function last12()
    {
        ...
        $ern = ("SELECT
                    ...
                    WHERE net_pay is not NULL 
                    AND date_submitted is not NULL
                    AND date_submitted > date_sub(NOW(), INTERVAL 13 MONTH)
                    GROUP BY
                        year_sub,
                        month_year
                    ORDER BY
